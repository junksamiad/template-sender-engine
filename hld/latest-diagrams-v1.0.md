# WhatsApp AI Chatbot - Latest Architecture Diagrams

This document contains the latest visual representations of the WhatsApp AI chatbot architecture, with a focus on the updated Channel Router implementation.
...
graph TD
    ...
    subgraph "AWS Cloud"
        ...
        WhatsAppEngine -->|Lookup Company Config| CompanyDB
        WhatsAppEngine -->|Create Conversation| ConvoDB
        WhatsAppEngine -->|Create Thread| OpenAI
        OpenAI -->|Function Call| WhatsAppEngine
        
        SecretsManager -.->|Provide Secrets| Router
...
graph TD
    ...
    subgraph "Channel Router"
        ...
        subgraph "Router Lambda (Concurrency: 25)"
    ...
    subgraph "Processing Engines"
        WL["WhatsApp Lambda<br/>(Batch Size: 1)"]
        EL["Email Lambda<br/>(Batch Size: 1)"]
        SL["SMS Lambda<br/>(Batch Size: 1)"]
...
## 3. Detailed Request Flow Sequence
...
sequenceDiagram
    ...
    Router->>Queue: Place message in appropriate channel queue
    ...
    Lambda->>DDB: Get company configuration
    DDB-->>Lambda: Return configuration
...
## 5. Enhanced DLQ Management System
...
## 6. Heartbeat Pattern for Long-Running Operations
...
graph TD
    ...
    subgraph "API Layer"
        APIG[API Gateway]
        THROTTLE["Throttling Config<br/>10 req/sec, 20 burst"]
    end
    
    subgraph "Compute Layer"
        ROUTER["Router Lambda"]
        CONCUR["Concurrency: 25"]
    ...
    WQ -.->|Failed Messages| WDLQ
    EQ -.->|Failed Messages| EDLQ
    SQ -.->|Failed Messages| SDLQ
    ...
    style ROUTER fill:#D6A2E8,stroke:#333,stroke-width:2px
    style CONCUR fill:#D6A2E8,stroke:#333,stroke-width:2px
    ...
    style SM fill:#D6EAF8,stroke:#333,stroke-width:2px
    style ROT fill:#D6EAF8,stroke:#333,stroke-width:2px
    style CW fill:#AED6F1,stroke:#333,stroke-width:2px
    style DASH fill:#AED6F1,stroke:#333,stroke-width:2px
    style ALARMS fill:#F5B7B1,stroke:#333,stroke-width:2px
    style OPENAI fill:#7DCEA0,stroke:#333,stroke-width:2px
    style TWILIO fill:#C39BD3,stroke:#333,stroke-width:2px
    style EMAIL fill:#C39BD3,stroke:#333,stroke-width:2px,stroke-dasharray: 5 5
    style SMS fill:#C39BD3,stroke:#333,stroke-width:2px,stroke-dasharray: 5 5
```

## 8. Database Schema with Rate Limiting Support
