...
def test_processor_failure_missing_secret(dynamodb_client, setup_e2e_company_data, modify_company_data_invalid_openai_secret):
    """
    ...
    # Define recipient_tel here as it's needed for polling and cleanup
    primary_channel = "+447111222333" # <-- Store primary_channel for cleanup
    payload = {
      "company_data": {
      ...
      "recipient_data": {
        ...
        "recipient_tel": primary_channel, # Use the variable
      ...
      "request_data": {
        "request_id": request_id, # <-- Use generated request_id
    ...
    conversation_record = None # Define conversation_record before try block
    try:
        ...
        final_item = poll_conversation_status(
            primary_channel_key=primary_channel, # Pass the phone number
        ...
        conversation_record = final_item # Store for cleanup in finally block
        ...
        pytest.fail(f"API request failed: {e}")
         ...
         pytest.fail(f"Assertion failed during test: {ae}")
    finally:
...
def test_processor_failure_invalid_twilio_number(dynamodb_client, setup_e2e_company_data):
    ...
    request_id = str(uuid.uuid4()) # <-- Generate unique request_id
    primary_channel = "+123" # Invalid E.164 format for Twilio - Store for cleanup
    ...
    payload = {
      ...
      "recipient_data": {
        ...
        "recipient_tel": primary_channel, # The invalid number
    ...
    try:
        ...
        logger.info("Polling DynamoDB conversations table for final status...")
        final_item = poll_conversation_status(
            primary_channel_key=primary_channel, # Use the invalid number as the key
            request_id=request_id,
        ...
        # Assert the polling was successful and the status is correct
        assert final_item is not None, \
            f"Polling timed out or failed to find record for request_id '{request_id}' with primary_channel '{primary_channel}'"

        conversation_record = final_item # Store for cleanup
...
def test_idempotency_e2e(dynamodb_client, setup_e2e_company_data):
    """
    ...
    final_item = None # Initialize conversation record placeholder
    try:
        # Send first request
        ...
        # Checks
        assert response_1.status_code == 200, f"First request failed with status {response_1.status_code}"
        assert response_1.json().get("request_id") == request_id, "Response didn't include correct request_id"
        
        # Give the system time to process the first request
        time.sleep(5)
        
        # Send identical second request (should be idempotent)
        ...
        # Checks for second request
        assert response_2.status_code == 200, f"Second request failed with status {response_2.status_code}"
        assert response_2.json().get("request_id") == request_id, "Second response didn't include correct request_id"
    ...
    # Exception handling
        ...
        pytest.fail(f"API request error: {e}")
